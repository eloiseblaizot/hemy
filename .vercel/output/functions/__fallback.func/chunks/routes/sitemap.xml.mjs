...
const sitemap_xml = defineEventHandler(async (event) => {
  ...
  const [scrutins, elus, groupes] = await Promise.all([
    prisma.scrutin.findMany({ select: { id: true, date: true }, orderBy: { date: "desc" }, take: 2e4 }),
...
${urls.map(
    (u) => `  <url><loc>${base}${u.loc}</loc>${u.lastmod ? `<lastmod>${u.lastmod}</lastmod>` : ""}<priority>${u.priority}</priority></url>`
  ).join("\n")}
</urlset>`;
